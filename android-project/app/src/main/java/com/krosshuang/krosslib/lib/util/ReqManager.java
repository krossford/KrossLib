...
public enum ReqManager {
    INSTANCE;

    private static final long DETECT_INTERVAL = 1000;

    private static final String LOG_TAG = "ReqManager";
    List<AbsAsyncTask> mReqList = new ArrayList<>();

    ReqManager() {
        //Log.i(LOG_TAG, "ReqManager.ReqManager constructor");
    ...
    private final Runnable checkTimeOut = new Runnable() {
        @Override
        public void run() {
            Log.i(LOG_TAG, "ReqManager.run check time out... req size: " + mReqList.size());
            ...
            if (!mReqList.isEmpty()) {
                ...
                Log.i(LOG_TAG, "ReqManager.run reqList is empty, died");
            }
        }
    };
